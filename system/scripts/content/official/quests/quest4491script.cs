...
public class Quest4491Script : QuestScript
{
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			if (character.Inventory.HasItem("MINE_2_CRYSTAL_10_ITEM", 1))
			{
				...
				dialog.AddonMessage("NOTICE_Dm_Clear", "The Magic Supply Device has been fixed{nl}Activate the Auxiliary Purifier");
			}
	...
	public override void OnStart(Character character, Quest quest)
	{
		...
		dialog.AddonMessage("NOTICE_Dm_Clear", "The Mine Compass is still pointing at District 4{nl}Go around District 4 and look for a way to remove the rust");
	}
}

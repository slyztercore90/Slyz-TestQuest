...
public class Quest20212Script : QuestScript
{
	protected override void Load()
	{
		...
		SetTrack("SProgress", "ESuccess", "REMAIN38_MQ05_TRACK", "m_boss_a");

		AddPrerequisite(new CompletedPrerequisite("REMAIN38_MQ04"));
		AddPrerequisite(new LevelPrerequisite(99));

		AddObjective("kill0", "Kill 1 Chapparition", new KillObjective(57104, 1));
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			...
			dialog.AddonMessage("NOTICE_Dm_Clear", "Defeat Chapparitions that are disturbing when you are checking the tombstone{nl}Check what was written on the last tombstone of Lydia Schaffen");

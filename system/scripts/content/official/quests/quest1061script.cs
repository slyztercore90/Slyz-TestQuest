...
public class Quest1061Script : QuestScript
{
	protected override void Load()
	{
		...
		SetTrack("SProgress", "ESuccess", "ROKAS29_VACYS4_TRACK", "m_boss_c");
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			if (character.Inventory.HasItem("ROKAS29_VACYS_ITEM_2", 1))
			{
				...
				dialog.AddonMessage("NOTICE_Dm_Clear", "It is written that the next document is hidden at Apatinis Cliff");
			}

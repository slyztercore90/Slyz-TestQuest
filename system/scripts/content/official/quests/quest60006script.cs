...
public class Quest60006Script : QuestScript
{
	protected override void Load()
	{
		...
		SetTrack("SProgress", "ESuccess", "VPRISON511_MQ_05_TRACK", "m_boss_b");

		AddPrerequisite(new CompletedPrerequisite("VPRISON511_MQ_04"));
		AddPrerequisite(new LevelPrerequisite(151));

		AddObjective("kill0", "Kill 1 Demon Lord Blut", new KillObjective(47514, 1));
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			...
			dialog.AddonMessage("NOTICE_Dm_Clear", "Meet Kupole Arune by moving to the 2nd area of Demons Prison!");

...
public class Quest8857Script : QuestScript
{
	protected override void Load()
	{
		...
		SetTrack("SProgress", "ESuccess", "FLASH64_MQ_03_TRACK", "m_boss_b");

		AddPrerequisite(new CompletedPrerequisite("FLASH64_MQ_02"));
		AddPrerequisite(new LevelPrerequisite(200));

		AddObjective("kill0", "Kill 1 Gargoyle", new KillObjective(57588, 1));
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			...
			dialog.AddonMessage("NOTICE_Dm_Clear", "Go to the Fortress of the Land by following the Grave Robber Amanda");
			return HookResult.Break;
		}

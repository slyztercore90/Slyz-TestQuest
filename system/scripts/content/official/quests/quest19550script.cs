...
public class Quest19550Script : QuestScript
{
	protected override void Load()
	{
		...
		SetTrack("SProgress", "ESuccess", "PILGRIM47_SQ_010_TRACK", 2000);

		AddPrerequisite(new LevelPrerequisite(124));

		AddObjective("kill0", "Kill 3 Jewel of Prominence(s)", new KillObjective(151002, 3));
	...
	private async Task<HookResult> BeforeEnd(Dialog dialog)
	{
		...
		if (character.Quests.IsCompletable(this.QuestId))
		{
			...
			await Task.Delay(5000);
			dialog.AddonMessage("NOTICE_Dm_Clear", "Successfully destroyed Tree Root Crystal! {nl}The curse laid around will disappear!");

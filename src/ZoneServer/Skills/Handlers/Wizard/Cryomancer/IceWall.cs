...
namespace Melia.Zone.Skills.Handlers.Cryomancer
{
	...
	public class IceWall : IDynamicGroundSkillHandler
	{
		...
		{
			Send.ZC_PLAY_SOUND(caster, caster.Gender == Gender.Male ? "voice_wiz_m_icewall_cast" : "voice_wiz_icewall_cast");
			Send.ZC_NORMAL.Skill_4D(caster, skill.Id);
		}

		public void HandleCastEnd(Skill skill, Character caster, float maxCastTime)
		{
			Send.ZC_STOP_SOUND(caster, caster.Gender == Gender.Male ? "voice_wiz_m_icewall_cast" : "voice_wiz_icewall_cast");

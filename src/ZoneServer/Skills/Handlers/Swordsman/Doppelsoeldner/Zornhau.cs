...
namespace Melia.Zone.Skills.Handlers.Doppelsoeldner
{
	...
	/// </summary>
	[SkillHandler(SkillId.Doppelsoeldner_Zornhau_Abil)]
	public class Zornhau : IGroundSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, ICombatEntity target)
		{
			...
			{
				Log.Warning("Zornhau: Player {0} cast skill farther than max range ({1} > {2}).", caster.Name, castRange, skill.Data.MaxRange);
				return;
			}
			...
			if (caster is Character character)
				Send.ZC_OVERHEAT_RESET_TIME(character, (int)skill.OverheatData.Id, skill.OverheatData.OverheatResetTime);
		}
	}
}

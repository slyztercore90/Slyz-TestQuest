...
namespace Melia.Zone.Skills.Handlers.Hoplite
{
	...
	public class Stabbing : IGroundSkillHandler
	{
		/// <summary>
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, ICombatEntity target)
		{
			...
			{
				Log.Warning("Stabbing: Player {0} cast skill farther than max range ({1} > {2}).", caster.Name, castRange, skill.Data.MaxRange);
				return;
			}
			...
			if (caster is Character character)
				Send.ZC_NORMAL.Skill_10B(character, skill.Id);
